...
def auth_is_valid(event):
    JWT_SECRET = ""
    headers = [x.lower() for x in event["headers"]]
    print(f"headers:\n{json.dumps(headers)}")
    if "jwt_token" in headers:
        JWT_TOKEN = ""
        for key, value in event["headers"].items():
            if key.lower() == "jwt_token":
                JWT_TOKEN = value
                break

        if not JWT_TOKEN:
            print("It shouldn't happen in this place in the code, but somehow the jwt token was not found")
            return False
        JWT_SECRET = os.environ["JWT_SECRET"]
